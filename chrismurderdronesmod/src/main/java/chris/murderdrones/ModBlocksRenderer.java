...
public class ModBlocksRenderer implements BlockEntityRenderer<PlushieBlockEntity> {
    ...
    public void render(PlushieBlockEntity blockEntity, float partialTick, PoseStack poseStack,
        ...
        BlockState state = blockEntity.getBlockState();
        if (!state.hasProperty(ModBlocks.ROTATION)) return;

        String character = getCharacter(blockEntity);
        int rotation     = state.getValue(ModBlocks.ROTATION);
        ...
        String modelPath = ModBlocks.getModelPath(character, modelName);
        ResourceLocation baseLocation = ResourceLocation.fromNamespaceAndPath(
                ChrisMurderDronesMod.MODID, "block/" + modelPath);
        ModelResourceLocation modelLoc = ModelResourceLocation.standalone(baseLocation);
        ...
        RenderType emissiveType = RenderType.eyes(
                ResourceLocation.fromNamespaceAndPath(
                        ChrisMurderDronesMod.MODID, "textures/block/" + textureName + "_e.png"));
