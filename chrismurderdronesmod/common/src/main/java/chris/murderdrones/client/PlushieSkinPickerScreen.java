...
import chris.murderdrones.network.SetPlushieSkinPayload;
...
public class PlushieSkinPickerScreen extends Screen {
    ...
    public PlushieSkinPickerScreen(BlockPos pos, CharacterEntry entry) {
        ...
        PlushieCategory owner = PlushieCatalog.findCategoryFor(character);
        ...
        if (button == 0) {
            for (int i = 0; i < cardBounds.size(); i++) {
                ...
                if (mouseX >= b[0] && mouseX < b[0] + b[2] && mouseY >= b[1] && mouseY < b[1] + b[3]
                    ...
                    Services.NETWORK.sendToServer(new SetPlushieSkinPayload(pos, skin));
